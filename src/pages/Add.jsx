...
// INSTANT visible bubbles - no delay!
const ProfessionalBubbles = () => {
  const [bubbles] = useState(() => {
    // Generate bubbles immediately on mount
    return Array.from({ length: 25 }, (_, i) => ({
      id: i,
      x: Math.random() * 100,
      // Distribute bubbles throughout the screen initially
      initialY: Math.random() * 120 - 10, // Start from -10vh to 110vh
    ...
    >
      {bubbles.map(bubble => (
        <motion.div
          ...
          initial={{
            x: `${bubble.x}vw`,
            y: `${bubble.initialY}vh`, // Start at distributed positions
            scale: 1,
            opacity: bubble.opacity // Start visible immediately
          }}
          animate={{
            y: [
              `${bubble.initialY}vh`,
              `${bubble.initialY - 125}vh` // Move up by 125vh
            ],
            x: [
              `${bubble.x}vw`,
...
export default function Form() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${host}/add`, {
        ...formData,
                        ...
                        InputProps={{
                          startAdornment: (
                            <InputAdornment position="start">
                              <motion.div
                                animate={{ 
                                  scale: focusedField === 'amount' ? [1, 1.2, 1] : 1 
                                }}
                                transition={{ 
                                  duration: 0.5,
